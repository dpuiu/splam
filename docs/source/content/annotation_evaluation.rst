...
.. admonition::  Here are some **optional arguments**:
    ...
    .. dropdown:: :code:`-f / --file-format FILE_FORMAT`
        ...
        Splam normally detects :code:`GFF` or :code:`GTF` from the input extension. Use this option to specify the format explicitly.

    .. dropdown:: :code:`-F / --features TYPES`
        :animate: fade-in-slide-down

        Provide a text file containing one parent feature type per line. Splam extracts introns beneath those feature types; the default is :code:`gene`.
...
.. admonition::  Here are some **optional arguments**:
    ...
    .. dropdown:: :code:`-t / --threshold threshold`
        ...
        This floating-point cutoff must be between 0 and 1. During annotation cleanup, Splam counts a junction as spurious when both its donor and acceptor scores are below the cutoff. The default is :code:`0.1`.

    .. dropdown:: :code:`-n / --bad-intron-num NUM`
        :animate: fade-in-slide-down

        Set the number of spurious introns used to classify a transcript as bad. The default is :code:`8`.

    .. dropdown:: :code:`-o / --outdir DIR`
        :animate: fade-in-slide-down

        The directory where the output file is written. Use the same directory supplied in :ref:`Step 2 <annotation-extract-introns>` and :ref:`Step 3 <annotation-score-introns>`. The default is :code:`tmp_out`.
